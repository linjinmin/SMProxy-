...
function packageSplit(string $data, bool $auth, int $headerLength = 4, bool $isClient = false, string &$halfPack = '')
{
    ...
    if ($dataLen == $packageLen) {
        // 如果数据长度 等于包头消息长度, 无数据丢失
        $halfPack = '';
        return [$data];
    } elseif ($dataLen < $packageLen) {
        // 如果数据长度 小于包头消息长度, 数据丢失, 等下一次包接受
        $halfPack = $data;
        return [];
    } else {
        // 如果数据长度 大于包头消息长度
 ...
 * 获取包长
 *
 * @param string $data 包体
 * @param int    $step 0开始
 *
 * @param int    $offset 登录3， 数据传送4
 *
 * a[0] = 0x78 a[1]=0x56 a[2]=0x34
 * 然后经过左移后就是a[0] = 0x78 a[1]=0x5600 a[2]=0x340000
 *
 * @return int
 */
